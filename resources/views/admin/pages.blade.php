			...
			<div class="card">
			  <div class="card-body">
				<div class="row card-title">
					<div class="col-sm-6">
						<h2 class="page-top-heading">CMS Lists</h2>
					</div>
					<div class="col-sm-6 pull-end text-right backbtndiv">
						<a class="btn btn-info pull-end" href="<?php echo URL::to('admin/addpage'); ?>"> + Add</a>
					</div>
				</div>
				<?php if(@$error_msg!=''){ ?>
				...
				<div class="tab-content p-3 border bg-light table-responsive" id="nav-tabContent">
					<table id="categorylist_datatable" class="table table-striped" style="width:100%">
						...
						<?php 
							if($allpages){
							 foreach($allpages as $ak=>$av){ ?>
							<tr id="rowid<?php echo $av->id; ?>">
								<td><?php echo $av->page_title; ?></td>
								<td><?php echo $av->slug; ?></td>
								<td><?php echo $av->short_description; ?> </td>
								<td><?php if($av->is_header==1){ echo 'Active';}else{ echo 'Inactive';} ?> </td>
								<td><?php if($av->is_footer==1){ echo 'Active';}else{ echo 'Inactive';} ?> </td>
								...
								<td>
									<a class="btn text text-info fa-2x" class="" href="<?php echo URL::to('admin/addpage/'.$av->id); ?>">Edit</a>
...
<script>
	...
	function removeCategory(id){
		$('#ajax_response').html('');
		$.ajax({
			url:"<?php echo URL::to('ajax/admin/removePage'); ?>",
			type:"POST",
			dataType:"JSON",
			data:{id:id},
			beforeSend:function(){
	...
	function changeStatus(id,st){
		$('#ajax_response').html('');
		$.ajax({
			url:"<?php echo URL::to('ajax/admin/changepagestatus'); ?>",
			type:"POST",
			dataType:"JSON",
			data:{id:id,st:st},
			beforeSend:function(){
