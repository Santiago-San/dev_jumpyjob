			...
			<div class="card">
			  <div class="card-body">
				<div class="row card-title">
					<div class="col-sm-6">
						<h2 class="page-top-heading">Faqs Lists</h2>
					</div>
					<div class="col-sm-6 pull-end text-right backbtndiv">
						<a class="btn btn-info pull-end" href="<?php echo URL::to('admin/addfaq'); ?>"> + Add</a>
					</div>
				</div>
				...
				<div class="tab-content p-3 border bg-light table-responsive" id="nav-tabContent">
					<table id="categorylist_datatable" class="table table-striped" style="width:100%">
						...
						<?php 
							if($allfaqs){
							 foreach($allfaqs as $ak=>$av){ ?>
							<tr id="rowid<?php echo $av->id; ?>">
								
								<td><?php echo $av->faq_title; ?></td>
								<td><?php echo $av->faq_answer; ?></td>
								...
								<td>
									<a class="btn text text-info fa-2x" class="" href="<?php echo URL::to('admin/addfaq/'.$av->id); ?>">Edit</a>
...
<script>
	...
	function removeCategory(id){
		$('#ajax_response').html('');
		$.ajax({
			url:"<?php echo URL::to('ajax/admin/removefaq'); ?>",
			type:"POST",
			dataType:"JSON",
			data:{id:id},
			beforeSend:function(){
	...
	function changeStatus(id,st){
		$('#ajax_response').html('');
		$.ajax({
			url:"<?php echo URL::to('ajax/admin/changefaqstatus'); ?>",
			type:"POST",
			dataType:"JSON",
			data:{id:id,st:st},
			beforeSend:function(){
