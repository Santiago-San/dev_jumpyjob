            ...
            <div class="profile-content">
				...
				<div class="row">
					...
					<div class="col-sm-6 pull-right text-end backbtndiv" >
						<a class="btn btn-info" href="<?php echo URL::to('jobs/categories'); ?>"> + Add</a>
					</div>
				</div>
					...
					<table id="categorylist_datatable" class="table table-striped" style="width:100%">
						...
						<?php 
							...
							<tr id="rowid<?php echo $av->id; ?>">
								...
								<td>
									<a class="btn text text-info fa-2x" class="" href="<?php echo URL::to('jobs/addcategory/'.$av->id); ?>"><i class="fas fa-pencil"></i></a>
...
<script>
	...
	function removeCategory(id){
		$('#ajax_response').html('');
		$.ajax({
			url:"<?php echo URL::to('ajax/removeJobCat'); ?>",
			type:"POST",
			dataType:"JSON",
			data:{id:id},
			beforeSend:function(){
	...
	function changeStatus(id,st){
		$('#ajax_response').html('');
		$.ajax({
			url:"<?php echo URL::to('ajax/changeCatSt'); ?>",
			type:"POST",
			dataType:"JSON",
			data:{id:id,st:st},
			beforeSend:function(){
